...
for element in list_of_directories:
    ...
    for j in range(0,prev.__len__()):
        if prev[j].endswith('.xlsx') is False and prev[j].endswith('.doc') is False and prev[j].endswith('.xls') is False:
            list_of_good_directories.append(prev[j])
            temp = os.listdir(source + '\\' + element + '\\' + prev[j])
            #print(temp)
            for k in range(0, temp.__len__()):
                #print(temp[k])
                reg = re.match("^[a-zA-Z][2,3,4][_][0-9][0-9][_][0-9][0-9]", temp[k])  # Dobry regex
                if reg and temp[k].endswith('xls') and temp[k].__contains__('BARCODE') is False:
                    list_of_good_files.append(temp[k])
                    size=list_of_good_files.__len__()
                    df = df.append(pd.read_excel(source+'\\'+element+'\\'+prev[j]+'\\'+list_of_good_files[size-1],skiprows=6))
